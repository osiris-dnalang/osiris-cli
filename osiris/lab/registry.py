...
class ExperimentRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "type": self.exp_type.value, "status": self.status.value,
            "script": self.script_path,
            "results": [r.result_path for r in self.results],
            "description": self.description,
            "backends": self.backends, "qubits": self.qubits, "shots": self.shots,
            "parameters": self.parameters, "key_findings": self.key_findings,
            "tags": self.tags,
        }
...
class ExperimentRegistry:
    ...
    def list_all(self, exp_type: Optional[ExperimentType] = None,
                 status: Optional[ExperimentStatus] = None,
                 tag: Optional[str] = None) -> List[ExperimentRecord]:
    ...
    def save(self):
        ...
        data = {
            "version": "1.0", "saved_at": time.time(),
            ...
            for eid, edata in data.get("experiments", {}).items():
                if eid not in self.experiments:
                    record = ExperimentRecord(
                        id=eid, name=edata.get("name", eid),
